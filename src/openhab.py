...
class OpenHab:
    ...
    def press(self, action_params):
        ...
        state = requests.get(base_url + "/state", headers={"content-type": "application/json"})
        try:
            current_idx = commands.index(state.text.upper())
        except ValueError:
            print(
                'Item {} now in state {}. But in "command" settings ({}) there is no such state.'.format(
                    action_params["item"], state.text, action_params["command"]
                )
            )
            return
